...
def is_between_100_and_999(user_input_number):
    ...
    if len(user_input_number)==0:
        result = False
    elif int(user_input_number) >= 100 and int(user_input_number)< 1000:
        result = True
    else:
        result = False
...
def is_duplicated_number(three_digit):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    comp = set([x for x in str(three_digit)])
    if len(comp) != len(str(three_digit)):
        result = True
    else:
        result = False
    # ==================================
    ...
    while start:
        ...
        if user_input == '0':
            ...
            if result[0]<3:
                ...
                while True:
                    quest = str(input("You win, one more(Y/N) ?"))
                    if is_yes(quest):
                        random_number = str(get_not_duplicated_three_digit_number())
                        print("Random Number is : ", random_number)
                        start = True
